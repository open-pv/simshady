...
To show that this position is more than theoretical, the CLI was used to simulate the PV yield for the entire city of Munich. The original data consists of 109 CityGML LoD2 tiles. Each tile had a size of 2x2 km^2. For processing purposed each tiles was split into four tiles, one square kilometre each. The data contains roughly 200 km² of building surface in total. Irradiance data from the National Solar Radiation Database [@sengupta2018national] was projected onto a HEALPix sky and ray-traced against the full geometry. The same CLI workflow that any user can apply to their own input data produced per-triangle yield artefacts for every tile: binary vertex/colour/intensity buffers, vertex-coloured OBJ meshes, orthographic top-down PNG snapshots, and JSON yield summaries. \autoref{fig:munich-tile} shows the 3D mesh output for one tile, and \autoref{fig:snapshot_topdown} shows the orthographic top-down PNG that the CLI generates for the full city.
